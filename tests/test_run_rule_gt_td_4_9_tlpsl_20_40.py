...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "rules" / "run_rule_gt_td_4_9_tlpsl_20_40.py"
RULE_NAME = "Rule-GT-TD-4-9-TLPSL-20-40"
REQUIRED_TD_COLUMNS = [
    "ADX_TD",
    "ATR_TD",
    "BOLLINGER_TD",
...
def load_script():
    spec = importlib.util.spec_from_file_location("run_rule_gt_td_4_9_tlpsl_20_40", SCRIPT_PATH)
...
def test_buy_call_tp_hit_uses_20_pips() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP"),
...
def test_sell_call_tp_hit_uses_20_pips() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "DOWN"),
...
def test_buy_call_sl_hit_uses_40_pips() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP"),
...
def test_sell_call_sl_hit_uses_40_pips() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "DOWN"),
...
def test_summary_totals_use_tp_close_win_sl_and_close_loss_pips() -> None:
    _, summary = run([
        ...
        row(3, 99.9, 100.05, 99.8, "DOWN"),
        row(4, 100.0, 100.05, 99.95, "UP"),
        row(5, 99.8, 100.05, 99.6, "UP"),
    ...
    assert summary["total_sell_call_tp_hit_pips"] == 20.0
    assert summary["total_buy_call_sl_hit_pips"] == 40.0
    ...
    assert summary["total_win_pip"] == 50.0
    assert summary["total_loss_pip"] == 50.0
...
def test_output_dir_rule_folder_is_used_directly_without_nested_duplicate() -> None:
    script = load_script()
    output_dir = Path(r"F:\GT-v1-shared-storage\rules\Rule-GT-TD-4-9-TLPSL-20-40")
