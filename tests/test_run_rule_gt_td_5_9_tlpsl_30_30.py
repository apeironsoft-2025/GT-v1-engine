...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "rules" / "run_rule_gt_td_5_9_tlpsl_30_30.py"
RULE_NAME = "Rule-GT-TD-5-9-TLPSL-30-30"
REQUIRED_TD_COLUMNS = [
    "ADX_TD",
    "ATR_TD",
    "BOLLINGER_TD",
...
def load_script():
    spec = importlib.util.spec_from_file_location("run_rule_gt_td_5_9_tlpsl_30_30", SCRIPT_PATH)
...
def five_buy_opposition() -> dict[str, str]:
    values = four_buy_opposition()
    values["ICHIMOKU_TD"] = "DOWN"
    return values
...
def five_sell_opposition() -> dict[str, str]:
    values = four_sell_opposition()
    values["ICHIMOKU_TD"] = "UP"
    return values
...
def test_td_opposition_does_not_close_when_only_4_of_9_are_opposite() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP"),
        row(1, 100.05, 100.10, 99.95, "UP", **four_buy_opposition()),
...
def test_td_opposition_closes_when_5_of_9_are_opposite() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP"),
        row(1, 100.1, 100.1, 100.0, "UP", **five_buy_opposition()),
    ])

    assert calls.iloc[0]["close_reason"] == "CLOSE_WIN"
    assert calls.iloc[0]["td_opposition_count"] == 5


def test_buy_call_closes_as_close_win_with_5_opposition_and_close_above_entry() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP"),
        row(1, 100.1, 100.1, 100.0, "UP", **five_buy_opposition()),
    ])
...
def test_buy_call_closes_as_close_loss_with_5_opposition_and_close_below_entry() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP"),
        row(1, 99.9, 100.0, 99.9, "UP", **five_buy_opposition()),
    ])
...
def test_sell_call_closes_as_close_win_with_5_opposition_and_close_below_entry() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "DOWN"),
        row(1, 99.9, 100.0, 99.9, "DOWN", **five_sell_opposition()),
    ])
...
def test_sell_call_closes_as_close_loss_with_5_opposition_and_close_above_entry() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "DOWN"),
        row(1, 100.1, 100.1, 100.0, "DOWN", **five_sell_opposition()),
    ])
...
def test_summary_totals_use_tp_close_win_sl_and_close_loss_pips() -> None:
    _, summary = run([
        ...
        row(7, 100.1, 100.1, 100.0, "UP", **five_buy_opposition()),
        row(8, 100.0, 100.05, 99.95, "DOWN"),
        row(9, 100.1, 100.1, 100.0, "DOWN", **five_sell_opposition()),
    ])
...
def test_markdown_documentation_is_created() -> None:
    temp_root = PROJECT_ROOT / ".tmp_rule_gt_td_5_9_tests" / uuid.uuid4().hex
    ...
    try:
        ...
        assert "at least 5 required TD columns" in markdown
...
def test_output_dir_rule_folder_is_used_directly_without_nested_duplicate() -> None:
    script = load_script()
    output_dir = Path(r"F:\GT-v1-shared-storage\rules\Rule-GT-TD-5-9-TLPSL-30-30")
