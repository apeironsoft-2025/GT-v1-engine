...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "rules" / "run_rule_gt_td_4_8_tlpsl_30_30.py"
...
def load_script():
    spec = importlib.util.spec_from_file_location("run_rule_gt_td_4_8_tlpsl_30_30", SCRIPT_PATH)
...
def buy_8_plus(extra_value: str) -> dict[str, str]:
    return {"ADX_TD": extra_value}


def sell_8_plus(extra_value: str) -> dict[str, str]:
    return {"ADX_TD": extra_value}
...
def four_buy_opposition() -> dict[str, str]:
    values = three_buy_opposition()
    values["EMA_STACK_TD"] = "NO_DIRECTION"
    return values
...
def test_rule_name_is_expected() -> None:
    script = load_script()
    assert script.RULE_NAME == RULE_NAME


def test_output_folder_is_rule_folder() -> None:
    script = load_script()
...
def test_buy_call_opens_with_8_up_and_1_down() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP", **buy_8_plus("DOWN")),
        row(1, 100.1, 100.3, 99.95, "UP"),
    ])

    assert calls.iloc[0]["call_type"] == "BUY_CALL"
    assert calls.iloc[0]["td_up_count_at_open"] == 8
    assert calls.iloc[0]["td_down_count_at_open"] == 1


def test_buy_call_opens_with_8_up_and_1_no_signal() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP", **buy_8_plus("NO_SIGNAL")),
        row(1, 100.1, 100.3, 99.95, "UP"),
    ])

    assert calls.iloc[0]["call_type"] == "BUY_CALL"
    assert calls.iloc[0]["td_no_signal_count_at_open"] == 1


def test_buy_call_does_not_open_with_8_up_and_1_no_direction() -> None:
    calls, _ = run([row(0, 100.0, 100.05, 99.95, "UP", **buy_8_plus("NO_DIRECTION"))])

    assert calls.empty


def test_sell_call_opens_with_8_down_and_1_up() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "DOWN", **sell_8_plus("UP")),
        row(1, 99.9, 100.05, 99.7, "DOWN"),
    ])

    assert calls.iloc[0]["call_type"] == "SELL_CALL"
    assert calls.iloc[0]["td_down_count_at_open"] == 8
    assert calls.iloc[0]["td_up_count_at_open"] == 1


def test_sell_call_opens_with_8_down_and_1_no_signal() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "DOWN", **sell_8_plus("NO_SIGNAL")),
        row(1, 99.9, 100.05, 99.7, "DOWN"),
    ])
...
def test_sell_call_does_not_open_with_8_down_and_1_no_direction() -> None:
    calls, _ = run([row(0, 100.0, 100.05, 99.95, "DOWN", **sell_8_plus("NO_DIRECTION"))])
...
def test_buy_call_opens_when_all_9_td_are_up() -> None:
    ...
    assert calls.iloc[0]["call_type"] == "BUY_CALL"
    assert calls.iloc[0]["td_up_count_at_open"] == 9
...
def test_sell_call_opens_when_all_9_td_are_down() -> None:
    ...
    assert calls.iloc[0]["call_type"] == "SELL_CALL"
    assert calls.iloc[0]["td_down_count_at_open"] == 9


def test_no_call_opens_when_only_7_td_are_same_direction() -> None:
    calls, _ = run([
        row(0, 100.0, 100.05, 99.95, "UP", ADX_TD="DOWN", ATR_TD="NO_SIGNAL"),
    ])
...
def test_summary_file_name_is_rule_name_based() -> None:
    script = load_script()
    _, summary_json, _ = script.output_paths(Path("sample_indicator_collection.csv"), Path("rules"))

    assert summary_json.name == f"{RULE_NAME}_SUMMARY.json"
...
def test_summary_totals_use_tp_close_win_sl_and_close_loss_pips() -> None:
    _, summary = run([
        row(0, 100.0, 100.05, 99.95, "UP", **buy_8_plus("DOWN")),
        row(1, 100.1, 100.3, 99.95, "UP"),
        row(2, 100.0, 100.05, 99.95, "DOWN", **sell_8_plus("UP")),
        row(3, 99.9, 100.05, 99.7, "DOWN"),
        row(4, 100.0, 100.05, 99.95, "UP", **buy_8_plus("DOWN")),
        row(5, 99.8, 100.05, 99.7, "UP"),
        row(6, 100.0, 100.05, 99.95, "UP", **buy_8_plus("DOWN")),
        row(7, 100.1, 100.1, 100.0, "UP", **four_buy_opposition()),
        row(8, 100.0, 100.05, 99.95, "DOWN", **sell_8_plus("UP")),
...
def test_markdown_documentation_is_created() -> None:
    temp_root = PROJECT_ROOT / ".tmp_rule_gt_td_4_8_tests" / uuid.uuid4().hex
    ...
    try:
        ...
        assert "all 9 required TD columns are UP" in markdown
        assert "exactly 8 required TD columns are UP" in markdown
...
def test_output_dir_rule_folder_is_used_directly_without_nested_duplicate() -> None:
    script = load_script()
    output_dir = Path(r"F:\GT-v1-shared-storage\rules\Rule-GT-TD-4-8-TLPSL-30-30")
    calls_csv, summary_json, markdown_path = script.output_paths(Path("sample.csv"), output_dir)
...
def test_validation_errors_are_clear() -> None:
    script = load_script()
    valid = df_from_rows(row(0, 100.0, 100.05, 99.95, "UP", **buy_8_plus("DOWN")))
