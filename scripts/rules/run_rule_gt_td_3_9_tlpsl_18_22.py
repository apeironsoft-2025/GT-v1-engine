"""Rule-GT-TD-3-9-TLPSL-18-22 backtest script.

Rule-GT-TD-3-9-TLPSL-18-22 is a TD-only 9-indicator agreement rule.

It opens one call only when there is no active call and all available *_TD
columns agree in the same direction. If all 9 *_TD columns are UP, it opens a
BUY_CALL at the current row Close. If all 9 *_TD columns are DOWN, it opens a
SELL_CALL at the current row Close.

It closes an open call by TP, SL, or TD opposition logic. Only one open call is
allowed at a time. Rows while a call is open are used only for close management.
"""
...
RULE_NAME = "Rule-GT-TD-3-9-TLPSL-18-22"
RULE_DESCRIPTION = (
    "Rule-GT-TD-3-9-TLPSL-18-22 is a TD-only 9-indicator agreement rule. "
...
SCRIPT_RELATIVE_PATH = "scripts\\rules\\run_rule_gt_td_3_9_tlpsl_18_22.py"
...
BUY_TP_DISTANCE = 0.220
BUY_SL_DISTANCE = 0.180
SELL_TP_DISTANCE = 0.220
SELL_SL_DISTANCE = 0.180
TP_PIPS = 22.0
SL_PIPS = 18.0
PRICE_EPSILON = 1e-9
...
def rule_output_dir(output_dir: str | Path) -> Path:
    return Path(output_dir) / RULE_NAME
...
def output_paths(input_path: Path, output_dir: str | Path) -> tuple[Path, Path, Path]:
    ...
    markdown_path = Path(output_dir) / f"{RULE_NAME}.md"
...
def validate_input_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    detected_td_columns = [str(column) for column in df.columns if str(column).endswith("_TD")]
    missing_td = [column for column in REQUIRED_TD_COLUMNS if column not in detected_td_columns]
    extra_td = [column for column in detected_td_columns if column not in REQUIRED_TD_COLUMNS]
    if missing_td or extra_td:
        details = []
        if missing_td:
            details.append("missing required TD columns: " + ", ".join(missing_td))
        if extra_td:
            details.append("unexpected TD columns: " + ", ".join(extra_td))
        raise ValueError("Input CSV TD columns invalid; " + "; ".join(details) + ".")
...
def actual_signed_pips(call_type: str, entry_price: float, close_price: float) -> float:
    if call_type == "BUY_CALL":
...
def evaluate_tp_sl(open_call: OpenCall, row: pd.Series) -> tuple[str, float] | None:
    ...
    if tp_hit and sl_hit:
        return "SL_HIT", -SL_PIPS
    if tp_hit:
        return "TP_HIT", TP_PIPS
    if sl_hit:
        return "SL_HIT", -SL_PIPS
    return None
...
def run_backtest(df: pd.DataFrame, source_file_name: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for row_index, row in data.reset_index(drop=True).iterrows():
        ...
        if td_opposition_count >= 2:
            signed_pips = actual_signed_pips(open_call.call_type, open_call.open_price, close)
    ...
    if open_call is not None:
        ...
        signed_pips = actual_signed_pips(open_call.call_type, open_call.open_price, final_close)
        calls.append(
            trace_record(
                source_file_name,
                open_call,
...
def build_summary(
    ...
    win_call_count = len(tp_hit) + len(close_win)
    loss_call_count = len(sl_hit) + len(close_loss)
    total_win_pip = win_call_count * 21
    total_loss_pip = loss_call_count * 19
    ...
    summary = {
        ...
        "required_td_columns": list(REQUIRED_TD_COLUMNS),
        ...
        "total_sell_call_close_loss_count": int(
            len(sell_calls[sell_calls["close_reason"] == "CLOSE_LOSS"])
        ),
        "total_sell_call_close_loss_pips": rounded(
            abs(sell_calls[sell_calls["close_reason"] == "CLOSE_LOSS"]["signed_pips"].sum())
        ),
        "total_sell_call_close_win_count": int(
            len(sell_calls[sell_calls["close_reason"] == "CLOSE_WIN"])
        ),
        "total_sell_call_close_win_pips": rounded(
            sell_calls[sell_calls["close_reason"] == "CLOSE_WIN"]["signed_pips"].sum()
        ),
        "total_buy_call_close_loss_count": int(
            len(buy_calls[buy_calls["close_reason"] == "CLOSE_LOSS"])
        ),
        "total_buy_call_close_loss_pips": rounded(
            abs(buy_calls[buy_calls["close_reason"] == "CLOSE_LOSS"]["signed_pips"].sum())
        ),
        "total_buy_call_close_win_count": int(
            len(buy_calls[buy_calls["close_reason"] == "CLOSE_WIN"])
        ),
        "total_buy_call_close_win_pips": rounded(
            buy_calls[buy_calls["close_reason"] == "CLOSE_WIN"]["signed_pips"].sum()
        ),
        "total_tp_hit_count": int(len(tp_hit)),
        "total_sl_hit_count": int(len(sl_hit)),
        "total_win_call_count": int(win_call_count),
        "total_loss_call_count": int(loss_call_count),
        "total_win_pip": int(total_win_pip),
        "total_loss_pip": int(total_loss_pip),
        "net_pip": int(total_win_pip - total_loss_pip),
        "actual_signed_pips": rounded(calls_df["signed_pips"].sum()) if len(calls_df) else 0.0,
...
## TP/SL Condition
BUY_CALL TP is High - entry_price >= 0.220. BUY_CALL SL is entry_price - Low >= 0.180. SELL_CALL TP is entry_price - Low >= 0.220. SELL_CALL SL is High - entry_price >= 0.180. Same-candle TP/SL conflict closes as SL_HIT.
