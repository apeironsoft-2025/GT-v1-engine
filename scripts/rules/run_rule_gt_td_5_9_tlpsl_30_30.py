"""Rule-GT-TD-5-9-TLPSL-30-30 is a TD-only 9-indicator agreement rule.
...
TD opposition close:
For BUY_CALL, close when at least 5 of the 9 *_TD columns are DOWN or NO_DIRECTION.
For SELL_CALL, close when at least 5 of the 9 *_TD columns are UP or NO_DIRECTION.
...
RULE_DESCRIPTION = (
    ...
    "TP, SL, or TD opposition logic with a 5-of-9 opposition threshold."
)
SCRIPT_RELATIVE_PATH = "scripts\\rules\\run_rule_gt_td_5_9_tlpsl_30_30.py"
...
def run_backtest(df: pd.DataFrame, source_file_name: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for row_index, row in data.reset_index(drop=True).iterrows():
        ...
        if open_call is None:
            call_type = open_call_type([row[column] for column in REQUIRED_TD_COLUMNS])
...
## Close Condition
Close management starts from the candle after the open candle. BUY_CALL closes by TD opposition when at least 5 required TD columns are DOWN or NO_DIRECTION. SELL_CALL closes by TD opposition when at least 5 required TD columns are UP or NO_DIRECTION.
