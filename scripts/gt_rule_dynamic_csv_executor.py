...
def parse_args():
    parser = argparse.ArgumentParser(description="Execute a dynamic GT rule JSON against a dataset CSV.")
...
def read_dataset(input_path, rule_columns):
    ...
    for original_index, row in enumerate(rows):
        ...
        normalized[rule_columns["macd_direction"]] = normalize_direction(row[rule_columns["macd_direction"]], rule_columns["macd_direction"], row_number)
        ...
        normalized[rule_columns["macd_strength"]] = normalize_float(row[rule_columns["macd_strength"]], rule_columns["macd_strength"], row_number)
...
def has_entry_signal(row, columns):
    ...
    return (
        ...
        and row[columns["macd_strength"]] == 1.0
        and row[columns["ema_stack_strength"]] == 1.0
    )
...
def evaluate_active_order(order, row, row_index, columns, rule_values):
    ...
    if tp_hit and sl_hit:
        return close_trade(
            ...
            row_index,
            columns,
            "HIT_TP_SL_IN_SAME_CANDLE",
            rule_values["same_candle_tp_sl_collected_pips"],
            row[columns["close"]],
        )
    if tp_hit:
        return close_trade(order, row, row_index, columns, "HIT_TP_WIN", rule_values["tp_pip"], order["tp_price"])
    if sl_hit:
        return close_trade(order, row, row_index, columns, "HIT_SL_LOSS", -rule_values["sl_pip"], order["sl_price"])
    ...
    if row[columns["macd_direction"]] != row[columns["ema_stack_direction"]]:
        ...
        status = status_from_pips("VIOLATE_TD_WIN", "VIOLATE_TD_LOSS", collected_pips)
        return close_trade(order, row, row_index, columns, status, collected_pips, close_price)
    if row[columns["macd_strength"]] != 1.0:
        collected_pips = round(calculate_collected_pips(order, close_price, rule_values["pip_size"]), 3)
        status = status_from_pips("VIOLATE_MACD_TS_WIN", "VIOLATE_MACD_TS_LOSS", collected_pips)
        return close_trade(order, row, row_index, columns, status, collected_pips, close_price)
    if row[columns["ema_stack_strength"]] != 1.0:
        collected_pips = round(calculate_collected_pips(order, close_price, rule_values["pip_size"]), 3)
        status = status_from_pips("VIOLATE_EMA_TS_WIN", "VIOLATE_EMA_TS_LOSS", collected_pips)
        return close_trade(order, row, row_index, columns, status, collected_pips, close_price)
...
def force_close_at_end(order, row, row_index, columns, rule_values):
    close_price = row[columns["close"]]
    collected_pips = round(calculate_collected_pips(order, close_price, rule_values["pip_size"]), 3)
    status = status_from_pips("FORCE_CLOSE_AT_END_WIN", "FORCE_CLOSE_AT_END_LOSS", collected_pips)
    return close_trade(order, row, row_index, columns, status, collected_pips, close_price)
...
def main():
    ...
    rule_values = {
        ...
        "same_candle_tp_sl_collected_pips": float(require_path(rule, "parameters.sameCandleTpSlCollectedPips")),
        "pip_size": pip_size,
    }
    ...
    print("GT dynamic rule CSV execution completed")
